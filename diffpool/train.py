...
def train(dataset, model, args, same_feat=True, val_dataset=None, test_dataset=None, writer=None,
    ...
    return model, val_accs, best_val_result
...
def benchmark_task_val(args, writer=None, feat='node-label'):
    ...
    if feat == 'node-feat' and 'feat_dim' in graphs[0].graph:
        ...
        for G in graphs:
            ...
            MISSING_RATE = float(os.environ.get("MISSING_RATE", 0.0))
            FILLING_METHOD = os.environ.get("FILLING_METHOD", "feature_propagation")

            edge_index = torch.tensor(np.array(G.edges).T).cuda()
            edge_index = to_undirected(edge_index)
            edge_index, _ = add_remaining_self_loops(edge_index, num_nodes=G.number_of_nodes())
            x = torch.tensor(np.array([util.node_dict(G)[u]['feat'] for u in G.nodes()])).float().cuda()
            missing_feature_mask = get_missing_feature_mask(rate=MISSING_RATE, n_nodes=G.number_of_nodes(), n_features=x.shape[1], type="uniform").cuda()
            filled_features = filling(FILLING_METHOD, edge_index, x, missing_feature_mask, 40)
            x = torch.where(missing_feature_mask, x, filled_features)
            for idx, u in enumerate(G.nodes()):
                util.node_dict(G)[u]['feat'] = x[idx].cpu().numpy()
    else:
        print('Using constant labels')
        featgen_const = featgen.ConstFeatureGen(np.ones(args.input_dim, dtype=float))
        for G in graphs:
    ...
    for i in range(10):
        ...
        _, val_accs, best_val_result = train(train_dataset, model, args, val_dataset=val_dataset, test_dataset=None,
            writer=writer)
        all_vals.append(np.array(val_accs))
        average_res.append(best_val_result['acc'])
        with open("results_log.txt", 'a+') as f:
            f.write(f"Best val acc {best_val_result['acc']}, missing rate {MISSING_RATE}, method {FILLING_METHOD}\n")
    all_vals = np.vstack(all_vals)
    all_vals = np.mean(all_vals, axis=0)
    print(all_vals)
    print(np.max(all_vals))
    print(np.argmax(all_vals))
    print("Mean best val acc", np.mean(average_res))
